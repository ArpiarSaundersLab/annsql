...
def expression_normalize_memory_wrapper():
	#normalize the expression
	asql.expression_normalize(total_counts_per_cell=1e4, chunk_size=950, print_progress=True) 

def expression_log_memory_wrapper():
	#calculate the log2 expression
	asql.expression_log(log_type="LN", chunk_size=950, print_progress=True)
...
def calculate_variable_genes_memory_wrapper():
	...
	asql.calculate_variable_genes(chunk_size=950, print_progress=True) 
...
def calculate_pca_memory_wrapper():
	#pca
	asql.calculate_pca(n_pcs=50, top_variable_genes=2000, chunk_size=950, zero_center=False, print_progress=True)
